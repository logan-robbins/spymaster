...
## PREREQUISITES - Bronze Backfill Status

**⚠️ IMPORTANT: Bronze backfill is COMPLETE as of 2025-12-23 ~10:45 PM**

The Bronze tier data at `backend/data/lake/bronze/` is the **canonical source** for all downstream processing. This data was generated by:

```bash
# Command that generated the Bronze data (ALREADY COMPLETED):
cd backend/
uv run python -m scripts.backfill_bronze_futures \
  --dates 2025-12-01,...,2025-12-19 \
  --clean \
  --batch-size 7500000 \
  --mbp10-batch-size 15000000 \
  --verbose
```

**Do NOT re-run the backfill unless instructed.** All validation and pipeline steps in this document use the existing Bronze data.

### What Was Backfilled (COMPLETED):

**ES Futures (from Databento DBN files):**
- ✅ Trades: `bronze/futures/trades/symbol=ES/date=YYYY-MM-DD/`
- ✅ MBP-10: `bronze/futures/mbp10/symbol=ES/date=YYYY-MM-DD/`
- ✅ Dates: 16 trading days (12-01 to 12-19, excluding weekends and 12-13)

**SPY Options (from Polygon S3 flat files):**
- ✅ Trades: `bronze/options/trades/underlying=SPY/date=YYYY-MM-DD/`
- ✅ Dates: 15 trading days (excluding 12-14 Saturday)

**Batch Sizes Used:**
- Trades: 7.5M rows per batch (optimized for 128GB RAM)
- MBP-10: 15M rows per batch (2x trades for massive 9GB files)

**YOUR TASK**: Validate this Bronze data, clean it (remove non-main ES contracts), then run the vectorized pipeline to generate Gold labels.

---

## 0) Bronze Data Inventory (USE THIS DATA)

### Bronze Data Location (ALREADY EXISTS)
```
backend/data/lake/bronze/
├── futures/
│   ├── trades/symbol=ES/date=YYYY-MM-DD/hour=HH/*.parquet  ✅ COMPLETE
│   └── mbp10/symbol=ES/date=YYYY-MM-DD/hour=HH/*.parquet   ✅ COMPLETE
└── options/
    └── trades/underlying=SPY/date=YYYY-MM-DD/*.parquet     ✅ COMPLETE
```

**This is your source data.** All subsequent steps (validation, filtering, vectorized pipeline, ML training) read FROM this Bronze data.

### What's In Bronze (Already Backfilled)
- **ES Futures Trades**: ~11M trades across 16 trading days
- **ES Futures MBP-10**: ~350M depth snapshots across 16 days  
- **SPY Options Trades**: ~11M 0DTE trades across 15 days
- **Date Coverage**: December 1-19, 2025 (weekdays only, 12-13 missing)
- **Storage Size**: ~35-40 GB total (compressed Parquet with ZSTD level 3)
...
## 1) Bronze Data Validation (Validate Existing Data)

**⚠️ DO NOT RE-RUN BACKFILL** - You are validating the existing Bronze data that was already created.

### 1.1 File Integrity Check

**Objective**: Verify all expected dates have complete data with no corrupted files in the EXISTING Bronze directory.
