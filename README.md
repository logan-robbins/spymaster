...
- Dynamic strike tracking (ATM ± 3 strikes) AND specific "Hotzone" surveillance
...
4. If `focus_strike` (Hotzone) is set, add `[Focus-1, Focus, Focus+1, Focus+2]`
5. Format tickers: `O:SPY{YYMMDD}{C/P}{STRIKE_8_DIGIT}`
6. Diff current vs target → return (add_list, remove_list)
...
- Hotzone Anchor: Optional user-defined pivot level (overrides ATM)
- Buckets: call_above, call_below, put_above, put_below (relative to Pivot)
...
- `POST /api/config/hotzone` - Set focus strike (Payload: `{"strike": 575.0}`)
