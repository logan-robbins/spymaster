"""
Optuna hyperparameter search for contrastive episode embeddings.

Key questions this answers:
1. Is our model architecture right-sized for ~1000 episodes?
2. What learning rate / temperature / regularization work best?
3. Should we train longer or shorter?
"""
...
class ContrastiveObjective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100, eta_min=lr / 100)
...
def run_search(level_type: str, n_trials: int = 100):
    lake_path = Path('lake')
    ...
    objective = ContrastiveObjective(lake_path, level_type, device)

    study = optuna.create_study(
        direction='maximize',
        study_name=f'contrastive_{level_type}',
    ...
    print(f'\n{"="*60}')
    print(f'Best trial for {level_type}:')
    ...
    with open(output_dir / f'{level_type.lower()}_optuna.json', 'w') as f:
