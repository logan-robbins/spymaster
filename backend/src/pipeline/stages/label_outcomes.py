...
def label_outcomes(
    ...
    - BREAK: Crossed 1 ATR in the direction of approach first
    - REJECT: Crossed 1 ATR in opposite direction first  
    - CHOP: Neither threshold crossed within horizon
    
    Direction semantics:
    - UP: approaching from below, BREAK means crossed above, REJECT means reversed down
    - DOWN: approaching from above, BREAK means crossed below, REJECT means reversed up
    
    Multi-timeframe: Computes outcomes independently at 2min, 4min, 8min horizons.
    ...
    Returns:
        DataFrame with outcome labels: outcome_2min, outcome_4min, outcome_8min
        and continuous outcomes: excursion_favorable, excursion_adverse
    """
    # Fixed horizons per IMPLEMENTATION_READY.md Section 3.3
    confirmation_windows = [120, 240, 480]  # 2min, 4min, 8min in seconds
    ...
    if 'atr' not in signals_df.columns:
        raise ValueError("Missing atr field (required for ATR-normalized thresholds).")
    
    level_prices = signals_df['level_price'].values.astype(np.float64)
    atrs = signals_df['atr'].values.astype(np.float64)  # Use pre-computed ATR from signal
    ...
    for window_sec, label in zip(confirmation_windows, window_labels):
        ...
        # Process each signal using first-crossing semantics (IMPLEMENTATION_READY.md Section 3.1)
        for i in range(n):
            ts = signal_ts[i]
            ...
            # Compute break and bounce (reject) thresholds based on direction
            if direction == 'UP':
                # Approaching from below: break = cross above, bounce = reverse down
                break_threshold = level_price + threshold_points
                bounce_threshold = level_price - threshold_points
                
                # Find first crossing times
                break_mask = window_high >= break_threshold
                bounce_mask = window_low <= bounce_threshold
            else:  # DOWN
                # Approaching from above: break = cross below, bounce = reverse up
                break_threshold = level_price - threshold_points
                bounce_threshold = level_price + threshold_points
            ...
            # Apply first-crossing semantics (IMPLEMENTATION_READY.md Section 3.1)
            if t_break is None and t_bounce is None:
                ...
                # Exact tie (same timestamp) - treat as CHOP per spec
                outcomes[i] = 'CHOP'
            
            # Compute continuous outcome variables (IMPLEMENTATION_READY.md Section 3.4)
            ...
            if direction == 'UP':
                ...
                excursion_favorable[i] = max(max_above, 0.0) / atr
                excursion_adverse[i] = max(max_below, 0.0) / atr
                ...
                excursion_favorable[i] = max(max_below, 0.0) / atr
                excursion_adverse[i] = max(max_above, 0.0) / atr
    ...
    # Add direction-aware continuous outcomes using 4min as primary
    # (IMPLEMENTATION_READY.md Section 3.4)
    if '4min' in results_by_window:
        ...
        result['outcome'] = primary_data['outcomes']  # Primary outcome
