...
def validate_compute_microstructure(date: str = "2025-06-04", level_name: str = "PM_HIGH"):
    """
    Validation script for ComputeMicrostructureStage.
    
    1. Runs prerequisites.
    2. Runs ComputeMicrostructureStage.
    3. Verifies Vacuum and Latency columns.
    """
    logger.info(f"Starting ComputeMicrostructure validation for date: {date}, level: {level_name}")
    ...
    # 2. Run Stage
    stage = ComputeMicrostructureStage()
    try:
    ...
    if not signals_df.empty:
        ...
        micro_cols = [
            'vacuum_duration_ms', 
            'replenishment_latency_bid_ms', 
            'replenishment_latency_ask_ms'
        ]
        
        missing_cols = [c for c in micro_cols if c not in signals_df.columns]
        
        if missing_cols:
             report["status"] = "failed_integrity"
             logger.error(f"Missing microstructure columns: {missing_cols}")
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_microstructure_report.json"
...
if __name__ == "__main__":
    validate_compute_microstructure()
