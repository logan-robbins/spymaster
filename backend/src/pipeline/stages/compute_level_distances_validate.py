...
def validate_compute_level_distances(date: str = "2025-06-04", level_name: str = "PM_HIGH"):
    """
    Validation script for ComputeLevelDistancesStage.
    
    1. Runs prerequisites.
    2. Runs ComputeLevelDistancesStage.
    3. Verifies distance columns.
    """
    logger.info(f"Starting ComputeLevelDistances validation for date: {date}, level: {level_name}")
    ...
    # 2. Run Stage
    stage = ComputeLevelDistancesStage()
    try:
    ...
    if not signals_df.empty:
        # Check Columns
        dist_cols = ['dist_to_level', 'dist_to_level_atr']
        missing_cols = [c for c in dist_cols if c not in signals_df.columns]
        
        if missing_cols:
             report["status"] = "failed_integrity"
             logger.error(f"Missing distance columns: {missing_cols}")
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_level_distances_report.json"
...
if __name__ == "__main__":
    validate_compute_level_distances()
