...
def validate_compute_level_walls(date: str = "2025-06-04", level_name: str = "PM_HIGH"):
    """
    Validation script for ComputeLevelWallsStage.
    
    1. Runs prerequisites.
    2. Runs ComputeLevelWallsStage.
    3. Verifies wall columns.
    """
    logger.info(f"Starting ComputeLevelWalls validation for date: {date}, level: {level_name}")
    ...
    if not signals_df.empty:
        ...
        if missing_cols:
             ...
             logger.error(f"Missing wall columns: {missing_cols}")
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_level_walls_report.json"
