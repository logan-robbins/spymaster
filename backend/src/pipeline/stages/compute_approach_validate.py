...
def validate_compute_approach(date: str = "2025-06-04", level_name: str = "PM_HIGH"):
    """
    Validation script for ComputeApproachFeaturesStage.
    
    1. Runs prerequisites.
    2. Runs ComputeApproachFeaturesStage.
    3. Verifies approach columns.
    """
    logger.info(f"Starting ComputeApproachFeatures validation for date: {date}, level: {level_name}")
    ...
    # 2. Run Stage
    stage = ComputeApproachFeaturesStage()
    try:
    ...
    if not signals_df.empty:
        ...
        app_cols = [
            ...
            'minutes_since_open', 'attempt_index', 'attempt_cluster_id'
        ]
        
        missing_cols = [c for c in app_cols if c not in signals_df.columns]
        
        if missing_cols:
             report["status"] = "failed_integrity"
             logger.error(f"Missing approach columns: {missing_cols}")
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_approach_report.json"
