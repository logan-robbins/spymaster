"""Stream session setup and execution for VP websocket transport."""
...
from .config import VPRuntimeConfig, build_config_from_mapping, resolve_config
...
from .stream_pipeline import ProducerLatencyConfig, async_stream_events
...
async def run_stream_session(
    *,
    ...
    logger.info(
        "VP stream connected: serving=%s serving_id=%s product_type=%s symbol=%s dt=%s start_time=%s radius=%d cell_width_ms=%d projection_horizons_bins=%s cfg=%s",
