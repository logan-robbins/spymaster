...
class AbsoluteTickEngine:
    ...
    def update(
        ...
        # Each entry: (price_int, add_delta, pull_delta, fill_delta)
        ...
        if action == ACTION_CLEAR:
            ...
            touched_info.append((price_int, float(size), 0.0, 0.0))
            ...
            if old_entry is not None:
                ...
                touched_info.append((cancel_price, 0.0, float(cancel_size), 0.0))
            else:
                self._cancel_order(order_id)
            ...
            if old_entry is not None:
                ...
                if old_price != price_int:
                    touched_info.append((old_price, 0.0, float(old_size), 0.0))
                    touched_info.append((price_int, float(size), 0.0, 0.0))
                else:
                    size_diff = size - old_size
                    if size_diff > 0:
                        touched_info.append(
                            (price_int, float(size_diff), 0.0, 0.0)
                        )
                    elif size_diff < 0:
                        touched_info.append(
                            (price_int, 0.0, float(-size_diff), 0.0)
                        )
                    else:
                        touched_info.append((price_int, 0.0, 0.0, 0.0))
            else:
                self._modify_order(order_id, side, price_int, size)
            ...
            if old_entry is not None:
                ...
                touched_info.append((fill_price, 0.0, 0.0, float(size)))
            else:
                self._fill_order(order_id, size)
        ...
        for p, add_delta, pull_delta, fill_delta in touched_info:
            idx = self._price_to_idx(p)
            if idx is None:
                continue
            ...
            self._rest_depth[idx] = float(
                self._depth_bid.get(p, 0) + self._depth_ask.get(p, 0)
            )
    ...
    def _add_order(
        ...
        # Incremental BBO: O(1) — new order can only improve BBO
        if side == SIDE_BID:
    ...
    def _cancel_order(self, order_id: int) -> None:
        ...
        if new_qty <= 0:
            ...
            # BBO repair: only scan when best level fully depleted
            if entry.side == SIDE_BID and entry.price_int == self._best_bid:
                self._best_bid = max(depth.keys()) if depth else 0
            elif entry.side == SIDE_ASK and entry.price_int == self._best_ask:
                self._best_ask = min(depth.keys()) if depth else 0
        else:
    ...
    def _modify_order(
        ...
        if old is not None:
            ...
            if new_qty <= 0:
                ...
                if old.side == SIDE_BID and old.price_int == self._best_bid:
                    self._best_bid = max(depth.keys()) if depth else 0
                elif old.side == SIDE_ASK and old.price_int == self._best_ask:
                    self._best_ask = min(depth.keys()) if depth else 0
            else:
    ...
    def sync_rest_depth_from_book(self) -> None:
        ...
        for price_int, qty in self._depth_bid.items():
            ...
            if idx is not None:
                self._rest_depth[idx] += float(qty)
    ...
    def export_book_state(self) -> bytes:
        ...
        state = {
            ...
            "depth_bid": dict(self._depth_bid),
            "depth_ask": dict(self._depth_ask),
