"""
Tape Engine: SPY prints confirm direction

Computes aggressor imbalance, velocity, and sweep detection near a level.

Based on PLAN.md §5.2

Interfaces consumed:
- event_types (Agent A): StockTrade, Aggressor
- market_state (Agent C): MarketState for windowed trade queries
...
from .market_state import MarketState, TimestampedTrade
...
class TapeEngine:
    """
    Computes tape momentum and aggression near a level.
    ...
    Interfaces consumed:
    - MarketState queries (Agent C) for windowed trades
    ...
    def compute_tape_state(
        ...
        """
        Compute tape metrics for a level.
        
        Args:
            level_price: The critical level being tested
            market_state: MarketState instance with trade buffers
        ...
        # Get trades near level for imbalance
        trades_near_level = market_state.get_trades_near_level(
            ts_now_ns=ts_now_ns,
            window_seconds=self.window_seconds,
            level_price=level_price,
            band_dollars=self.tape_band_dollars
        )
        ...
        # Get all recent trades for velocity
        all_trades = market_state.get_trades_in_window(
            ts_now_ns=ts_now_ns,
        ...
        # Detect sweeps
        sweep = self._detect_sweep(trades_near_level, level_price)
        
        # Compute confidence
        total_vol = buy_vol + sell_vol
        confidence = min(1.0, total_vol / 50000.0)  # Scale to 1.0 at 50k shares
    ...
    def _compute_imbalance(
        ...
        """
        Compute buy and sell volume from trades.
        
        Args:
            trades: List of timestamped trades
            
        Returns:
            (buy_vol, sell_vol) in shares
        """
        buy_vol = 0
        sell_vol = 0
    ...
    def _detect_sweep(
        ...
        Args:
            trades: List of timestamped trades near level
        ...
        for cluster in clusters:
            ...
            total_notional = sum(t.price * t.size for t in cluster)
    ...
    def _find_trade_clusters(
        self,
        trades: List[TimestampedTrade]
    ) -> List[List[TimestampedTrade]]:
        """
        Group trades into clusters based on time gaps.
        
        Args:
            trades: List of timestamped trades (assumed sorted by time)
