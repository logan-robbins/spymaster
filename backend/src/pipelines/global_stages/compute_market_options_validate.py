...
def validate_compute_market_options(date: str = "2025-06-04"):
    """
    Validation script for ComputeMarketOptionsStage.
    
    1. Runs prerequisites.
    2. Runs ComputeMarketOptionsStage.
    3. Verifies global GEX and Tide columns.
    """
    logger.info(f"Starting ComputeMarketOptions validation for date: {date}")
    ...
    # 2. Run Stage
    stage = ComputeMarketOptionsStage()
    try:
    ...
    if not signals_df.empty:
        # Check Columns
        gex_cols = ['total_gex', 'gex_asymmetry', 'total_call_gex', 'total_put_gex']
        tide_cols = ['call_tide', 'put_tide']
        vol_cols = ['total_call_volume', 'total_put_volume', 'put_call_ratio']
        
        all_cols = gex_cols + tide_cols + vol_cols
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_market_options_report.json"
...
if __name__ == "__main__":
    validate_compute_market_options()
