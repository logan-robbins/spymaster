...
def validate_compute_market_micro(date: str = "2025-06-04"):
    """
    Validation script for ComputeMarketMicroStage.
    
    1. Runs prerequisites.
    2. Runs ComputeMarketMicroStage.
    3. Verifies global microstructure columns.
    """
    logger.info(f"Starting ComputeMarketMicro validation for date: {date}")
    ...
    if not signals_df.empty:
        # Check Columns
        micro_cols = ['spread', 'spread_pct', 'bid_depth', 'ask_depth', 'depth_imbalance', 'mid_price']
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_market_micro_report.json"
