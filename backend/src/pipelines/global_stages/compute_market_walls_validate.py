...
def validate_compute_market_walls(date: str = "2025-06-04"):
    """
    Validation script for ComputeMarketWallsStage.
    
    1. Runs prerequisites.
    2. Runs ComputeMarketWallsStage.
    3. Verifies global wall columns.
    """
    logger.info(f"Starting ComputeMarketWalls validation for date: {date}")
    ...
    ctx = ComputeMarketOptionsStage().run(ctx) # Needed for Options data/walls context sometimes, or just ensure option_trades in ctx
    
    # 2. Run Stage
    stage = ComputeMarketWallsStage()
    try:
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_market_walls_report.json"
