...
def validate_compute_barrier_evolution(date: str = "2025-06-04", level_name: str = "PM_HIGH"):
    """
    Validation script for ComputeBarrierEvolutionStage.
    
    1. Runs prerequisites.
    2. Runs ComputeBarrierEvolutionStage.
    3. Verifies barrier columns (delta, pct_change).
    """
    logger.info(f"Starting ComputeBarrierEvolution validation for date: {date}, level: {level_name}")
    ...
    # 2. Run Stage
    stage = ComputeBarrierEvolutionStage()
    try:
    ...
    if not signals_df.empty:
        # Check Columns
        windows = [1, 2, 3, 5]
        barrier_cols = ['barrier_depth_current']
        for w in windows:
            suffix = f"_{int(w)}min"
            barrier_cols.append(f"barrier_delta{suffix}")
            barrier_cols.append(f"barrier_pct_change{suffix}")
            
        missing_cols = [c for c in barrier_cols if c not in signals_df.columns]
        
        if missing_cols:
             report["status"] = "failed_integrity"
             logger.error(f"Missing barrier columns: {missing_cols}")
        
        # Stats
        stats = {}
        target_cols = ['barrier_depth_current', 'barrier_delta_1min', 'barrier_pct_change_1min']
    ...
    # Write Report
    report_path = Path(__file__).parent / "compute_barrier_evolution_report.json"
...
if __name__ == "__main__":
    validate_compute_barrier_evolution()
