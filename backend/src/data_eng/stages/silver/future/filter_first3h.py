...
class SilverFilterFirst3Hours(Stage):
    """Silver stage: keep only the first 3 hours of RTH (09:30–12:30 NY).

    - Input:  silver.future.market_by_price_10_session_levels (with PM_HIGH/PM_LOW computed)
    - Output: silver.future.market_by_price_10_first3h

    Filtering is based on `ts_event_est` (America/New_York).
    Core requirement: only first 3 hours matter for retrieval/trading.
    """

    def __init__(self) -> None:
        super().__init__(
            name="silver_filter_first3hours",
            io=StageIO(
