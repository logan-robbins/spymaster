...
class SilverComputeRadarVacuum1s(Stage):
    def __init__(self) -> None:
        super().__init__(
            ...
            io=StageIO(
                inputs=["bronze.future_mbo.mbo", "silver.future_mbo.book_snapshot_1s"],
    ...
    def transform(self, df_mbo: pd.DataFrame, df_snap: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        # Filter MBO for symbol (just in case)
        ...
        # Build Spot Map: window_end_ts -> spot_ref_price_int
        # df_snap has 'window_end_ts_ns' and 'spot_ref_price_int'
...
def compute_radar_vacuum_1s(df: pd.DataFrame, spot_map: Dict[int, int]) -> pd.DataFrame:
    ...
    for i in range(len(df)):
        ...
        if curr_window_id is None:
            ...
            # Init start state for first window
            curr_spot_ref = spot_map.get(window_end_ts, 0)
            if curr_spot_ref > 0:
                start_snap = _snapshot(orders, curr_spot_ref)
            else:
                start_snap = None
            
        elif w_id != curr_window_id:
            # End of Window
            # If we had a valid spot ref, emit
        ...
        if act == "R":
            orders.clear()
            accum = _new_accumulators()
            # If R happens, existing start_snap is technically invalid for delta?
            # Or we just accept the massive change. 
            # Ideally we mark window invalid. But let's follow existing patterns.
            # If orders cleared, start_snap (from past) vs end_snap (empty) -> massive delta.
            continue
            
        oid = oid_arr[i]
        old_order = orders.get(oid)
        
        # We need to reconstruct "bucket" logic on the fly relative to curr_spot_ref
        # Logic matches compute_level_vacuum_5s but "bucket" is derived, not stored.
        
        if curr_spot_ref <= 0:
            # Update orders only, no accum/physics if no reference
            _update_orders_simple(orders, act, oid, side_arr[i], price_arr[i], size_arr[i], ts)
            continue
            
        # Get Old State Helper
        if old_order:
        ...
        has_new = False # Is there a resulting order?
        ...
        if act == "A":
            ...
            # If price changed, reset ts_enter
            if new_px != old_px:
                new_ts_enter = ts
            ...
            if new_qty > 0:
                orders[oid].qty = new_qty # Update in place
                # Bucket/Price/Side same
                has_new = True
            else:
                orders.pop(oid, None)
                has_new = False
        ...
        # Delta Analysis for Accumulators
        ...
        if delta_ask > 0: accum["ask_add_qty"] += float(delta_ask)
        if delta_bid > 0: accum["bid_add_qty"] += float(delta_bid)
        
        # Pulls (C or M reducing size or moving away)
        # Note: M with price change is effectively C old + A new logic in LevelVacuum?
        # compute_level_vacuum_5s treats M as:
        # new_bucket = bucket_for(new_px)
        # IF new_bucket != old_bucket: new_bucket_enter_ts = ts
        # Then calculates delta Q.
        
        # Resting Logic used ts_event - old.bucket_enter_ts.
        # Here we use ts_event - old.ts_enter_price.
        ...
        if delta_ask < 0 and act in PULL_ACTIONS:
            ...
            if age >= REST_NS:
                ...
                if old_bucket == ASK_ABOVE_AT: accum["ask_pull_rest_qty_at"] += pull
                elif old_bucket == ASK_ABOVE_NEAR: accum["ask_pull_rest_qty_near"] += pull
        ...
        if delta_bid < 0 and act in PULL_ACTIONS:
            ...
            if age >= REST_NS:
                ...
                if old_bucket == BID_BELOW_AT: accum["bid_pull_rest_qty_at"] += pull
                elif old_bucket == BID_BELOW_NEAR: accum["bid_pull_rest_qty_near"] += pull

        # Reprice Logic (M only)
        if act == "M":
             # Ask Reprice
            if old_side == "A" and old_in_ask:
                ...
                if age >= REST_NS:
                    ...
                    # Note: Using distance in INT
                    
                    if new_px <= curr_spot_ref:
                        # Repriced into/below spot (aggression?)
    ...
    # Approach Dir (Spot vs Trends) -> requires trade stream or snapshot?
    # compute_level_vacuum_5s uses trade stream.
    # We can reconstruct it or just use spot_ref changes?
    # spot_ref IS last trade price (mostly).
    # Let's use spot_ref history to determine "approach_dir" for consistency?
    # Actually, compute_level_vacuum_5s checks if *Price* is approaching *Level*.
    # Here, "Level" IS "Price" (Spot). So approach is always 0?
    # No, "approach_dir" in Radar context means: Is the market trending UP or DOWN?
    # IMPLEMENT.md says: "approach_dir (derived from MBO trade prints / trend; must not be used to null the output)"
    # We can use the same logic: trend of spot.
...
def _update_orders_simple(orders, act, oid, side, price, size, ts):
    if act == "A":
        orders[oid] = OrderState(side, price, size, ts)
    elif act == "C":
        orders.pop(oid, None)
    elif act == "M":
        # Check if price changed for ts update
        old = orders.get(oid)
...
def _bucket_for(side: str, price_int: int, spot_ref: int) -> str:
    if side == "A" and price_int >= spot_ref:
        ...
        if ticks < 0 or ticks > DELTA_TICKS: return BUCKET_OUT
        if ticks <= AT_TICKS: return ASK_ABOVE_AT
        if ticks <= NEAR_TICKS: return ASK_ABOVE_NEAR
        if ticks >= FAR_TICKS_LOW: return ASK_ABOVE_FAR
        return ASK_ABOVE_MID
        
    if side == "B" and price_int <= spot_ref:
        ticks = int(round((spot_ref - price_int) / TICK_INT))
        if ticks < 0 or ticks > DELTA_TICKS: return BUCKET_OUT
        if ticks <= AT_TICKS: return BID_BELOW_AT
        if ticks <= NEAR_TICKS: return BID_BELOW_NEAR
        if ticks >= FAR_TICKS_LOW: return BID_BELOW_FAR
...
def _snapshot(orders: Dict[int, OrderState], spot_ref: int) -> Dict[str, float]:
    d = {
        "ask_depth_total": 0.0, "ask_depth_at": 0.0, "ask_depth_near": 0.0, "ask_depth_far": 0.0,
        "bid_depth_total": 0.0, "bid_depth_at": 0.0, "bid_depth_near": 0.0, "bid_depth_far": 0.0,
    }
    
    # Com calc accumulators
    ask_com_num = 0.0
    bid_com_num = 0.0
    min_ask = None
    max_bid = None
    ...
    for o in orders.values():
        ...
        if bucket == BUCKET_OUT: continue
        
        qty = float(o.qty)
        if o.side == "A":
            d["ask_depth_total"] += qty
            ask_com_num += float(o.price_int) * qty
            if bucket == ASK_ABOVE_AT: d["ask_depth_at"] += qty
            elif bucket == ASK_ABOVE_NEAR: d["ask_depth_near"] += qty
            elif bucket == ASK_ABOVE_FAR: d["ask_depth_far"] += qty
            
            if min_ask is None or o.price_int < min_ask: min_ask = o.price_int
            
        else:
            d["bid_depth_total"] += qty
            bid_com_num += float(o.price_int) * qty
            if bucket == BID_BELOW_AT: d["bid_depth_at"] += qty
            elif bucket == BID_BELOW_NEAR: d["bid_depth_near"] += qty
            elif bucket == BID_BELOW_FAR: d["bid_depth_far"] += qty
            
            if max_bid is None or o.price_int > max_bid: max_bid = o.price_int
    ...
    if min_ask is None: d["bbo_ask_ticks"] = float(DELTA_TICKS)
    else: d["bbo_ask_ticks"] = max((min_ask - spot_ref) / TICK_INT, 0.0)
    
    if max_bid is None: d["bbo_bid_ticks"] = float(DELTA_TICKS)
    else: d["bbo_bid_ticks"] = max((spot_ref - max_bid) / TICK_INT, 0.0)
    
    return d

def _new_accumulators() -> Dict[str, float]:
    return {
        "ask_add_qty": 0.0, "ask_pull_rest_qty": 0.0,
        "ask_pull_rest_qty_at": 0.0, "ask_pull_rest_qty_near": 0.0,
        "ask_reprice_away_rest_qty": 0.0, "ask_reprice_toward_rest_qty": 0.0,
        "bid_add_qty": 0.0, "bid_pull_rest_qty": 0.0,
        "bid_pull_rest_qty_at": 0.0, "bid_pull_rest_qty_near": 0.0,
        "bid_reprice_away_rest_qty": 0.0, "bid_reprice_toward_rest_qty": 0.0,
    }

# Copied Features Logic (Identical to LevelVacuum)
...
def _compute_approach_dir_simple(spot_history: np.ndarray) -> list:
    # Trend Analysis on Spot History
    # Simple differencing
    if len(spot_history) == 0: return []
    res = np.full(len(spot_history), "approach_none", dtype=object)
    
    # Needs valid window history? 
    # Logic: trend = p[t] - p[t-3] (approx 3s trend)
    # Since granularity is 1s, t-3 is 3 seconds ago.
    
    if len(spot_history) > 3:
        trend = spot_history[3:] - spot_history[:-3]
        # Map to "approach_up" if trend > 0, "approach_down" if trend < 0
        # What about "approach_dir" vs Level?
        # In radar, the "Level" is the spot point itself... so it's always "approaching itself"?
        # No, approach_dir describes the MOMENTUM direction.
        # IMPLEMENT.md: "derived from MBO trade prints / trend"
        
        # Valid indices start at 3
        # trend[0] corresponds to spot_history[3] - spot_history[0]
        
        trends_cat = np.where(trend > 0, "approach_up", 
                              np.where(trend < 0, "approach_down", "approach_none"))
        res[3:] = trends_cat
    
    return res.tolist()

def _validate_feature_ranges(df: pd.DataFrame):
     # No-op or log warnings
     pass
