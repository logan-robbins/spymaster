...
class BronzeIngestEquityMbo(Stage):
    def __init__(self) -> None:
        super().__init__(
            name="bronze_ingest_equity_mbo",
            io=StageIO(
                inputs=[],
                output="bronze.equity_mbo.mbo",
            ),
        )
    ...
    def run(self, cfg: AppConfig, repo_root: Path, symbol: str, dt: str, product: ProductConfig | None = None) -> None:
        ...
        session_start_ns, session_end_ns = session_window_ns(dt, product_type="equity_mbo")
        ...
        # session/snapshot boundaries.  Both must survive the time filter.
        F_SNAPSHOT = 32
