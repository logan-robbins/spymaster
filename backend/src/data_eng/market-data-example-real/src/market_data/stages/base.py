...
class Stage:
    ...
    def run(self, cfg: AppConfig, repo_root: Path, dt: str) -> None:
        out_ref = partition_ref(cfg, self.io.output, dt)
        if is_partition_complete(out_ref):
            return

        # Ensure all inputs exist and are complete.
        inputs_refs = [partition_ref(cfg, k, dt) for k in self.io.inputs]
