...
def partition_ref(cfg: AppConfig, dataset_key: str, dt: str) -> PartitionRef:
    spec = cfg.dataset(dataset_key)
    # This demo only supports partitioning by dt.
    if spec.partition_keys != ["dt"]:
        raise ValueError(f"Demo only supports partition_keys=['dt'], got {spec.partition_keys}")
    return PartitionRef(
        dataset_key=dataset_key,
        dir=cfg.lake_root / spec.path / f"dt={dt}",
    )
...
def write_partition_csv(
    ...
    out = partition_ref(cfg, dataset_key, dt)
