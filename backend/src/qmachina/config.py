"""Single-instrument runtime configuration for qMachina.
...
class RuntimeConfig:
    """Resolved runtime instrument configuration for qMachina."""
...
def _load_locked_instrument_config(path: Path) -> RuntimeConfig:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            f"Expected: {path}\n"
            f"Override with env: QMACHINA_INSTRUMENT_CONFIG_PATH"
        )
