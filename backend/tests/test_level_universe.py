...
Updated to use ES-based MarketState interface.
"""
...
def make_es_trade(spy_price: float, ts_ns: int = 1000000000, size: int = 1) -> FuturesTrade:
    """Helper to create ES trade at SPY-equivalent price."""
    return FuturesTrade(
        ts_event_ns=ts_ns,
        ts_recv_ns=ts_ns,
        source=EventSource.SIM,
        symbol="ES",
        price=spy_price * 10,  # ES price = SPY * 10
        size=size,
...
class TestLevelUniverse:
    ...
    def test_generates_vwap_level(self):
        ...
        # Add ES trade to establish spot and VWAP (545.50 SPY = 5455.0 ES)
        market_state.update_es_trade(make_es_trade(545.50))
        ...
        assert abs(vwap_levels[0].price - 545.50) < 0.01
    ...
    def test_generates_round_levels_near_spot(self):
        ...
        market_state.update_es_trade(make_es_trade(545.42))
    ...
    def test_generates_strike_levels_from_option_flows(self):
        ...
        # Add spot
        market_state.update_es_trade(make_es_trade(545.00))
...
class TestIntegration:
    ...
    def test_full_workflow(self):
        ...
        # Should have VWAP, rounds, strikes, and walls
        kinds = set(l.kind for l in levels)
        assert LevelKind.VWAP in kinds
        assert LevelKind.ROUND in kinds
        assert LevelKind.STRIKE in kinds
        assert LevelKind.CALL_WALL in kinds
        assert LevelKind.PUT_WALL in kinds

        # Find nearest level to spot (545.42)
        nearest = min(levels, key=lambda l: abs(l.price - 545.42))

        # Compute runway
        rtr = RoomToRun()
        runway_up, runway_down = rtr.compute_bidirectional_runway(
            nearest, levels, spot=545.42
        )

        # Should have runway in both directions
        assert runway_up.direction == Direction.UP
        assert runway_down.direction == Direction.DOWN
        print(f"Nearest level: {nearest.id} @ {nearest.price}")
        print(f"Runway UP: {runway_up.distance} to {runway_up.next_obstacle.id if runway_up.next_obstacle else 'none'}")
        print(f"Runway DOWN: {runway_down.distance} to {runway_down.next_obstacle.id if runway_down.next_obstacle else 'none'}")
