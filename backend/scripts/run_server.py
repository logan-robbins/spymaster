"""Run the qMachina WebSocket server (serving parity mode)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="qMachina stream server (serving alias/ID required per client)",
    )
    ...
    if args.example_serving.strip():
        ...
        print(
            "  WebSocket: "
            f"ws://localhost:{args.port}/v1/stream?serving={serving}"
        )
        print(
            "  Frontend:  "
        ...
        print(
            "  WebSocket: "
            f"ws://localhost:{args.port}/v1/stream?serving=<alias_or_id>"
        )
        print(
            "  Frontend:  "
