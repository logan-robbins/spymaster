...
def _existing_parquet_for_date(data_root: Path, schema_path: str, date: str) -> bool:
    """
    Check if any parquet exists for a given date across all symbol partitions.
    
    This avoids silently mixing different contract symbols into a single
    symbol=ES partition.
    """
    root = data_root / "bronze" / schema_path
    pattern = root.glob(f"symbol=*/date={date}/**/*.parquet")
    return any(pattern)


def _clean_date_partitions(data_root: Path, schema_path: str, date: str) -> None:
    """
    Remove all partitions for a date across all symbols/contracts.
    """
    root = data_root / "bronze" / schema_path
    if not root.exists():
        return
    for date_dir in root.glob(f"symbol=*/date={date}"):
        if date_dir.exists():
...
def backfill_mbp10(
    ...
    for mbp10 in dbn.read_mbp10(
        ...
        # Partition by actual contract symbol to avoid mixing contracts.
        record["_partition_key"] = record["symbol"]
        batch.append(record)
        count += 1
...
def process_single_date(args_tuple):
    ...
    Returns:
        Dict with status
    """
    (date, data_root, symbol_prefix, batch_size, force, start_ns, end_ns, log_every, verbose) = args_tuple
    
    try:
        dbn = DBNReader()
        
        # Check if data exists for this date across any symbol partition
        if _existing_parquet_for_date(data_root, "futures/mbp10", date):
            if not force:
                return {'date': date, 'status': 'error', 'error': 'parquet exists; rerun with --force to overwrite'}
            # Clean all partitions for that date before rewriting to keep idempotency
            _clean_date_partitions(data_root, "futures/mbp10", date)
...
def main() -> int:
    ...
    parser.add_argument("--batch-size", type=int, default=5_000_000, help="Batch size for MBP-10 writes (default: 5M)")
    parser.add_argument("--verbose", action="store_true", help="Log progress during backfill")
    parser.add_argument("--log-every", type=int, default=1_000_000, help="Log progress every N rows")
    ...
    if args.clean:
        for date in dates:
            # Clean all partitions for the date (all contract symbols)
            _clean_date_partitions(data_root, "futures/mbp10", date)
            print(f"  Cleaned existing MBP-10 partitions for {date}")
    ...
    job_args = [
        (date, data_root, args.symbol, args.batch_size, args.force,
         args.start_ns, args.end_ns, args.log_every, args.verbose)
        for date in dates
    ]
