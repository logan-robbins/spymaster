...
def main():
    ...
    try:
        # Load checkpoint from stage 1 (should already exist from pipeline run)
        logger.info(f"Loading checkpoint from stage 1 for level {args.level}...")
        from src.pipeline.core.checkpoint import CheckpointManager
        manager = CheckpointManager(args.checkpoint_dir)
        ctx = manager.load_checkpoint("bronze_to_silver", args.date, stage_idx=1, level=args.level)
