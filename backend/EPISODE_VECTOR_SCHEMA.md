...
**Purpose**: 185-dimensional "source of truth" vectors for similarity retrieval and ML training. Vectors are compressed to 32D (Geometry Only) for FAISS storage per Phase 4 calibration.
...
## Section A: Context & Regime (59 dimensions)
...
**Per-Level Context (36 features)**: Captures simultaneous multi-level dynamics across all 6 levels. Touch detection uses OHLC range intersection with TOUCH_BAND (±2 points). Defended touches = level held (touched but did NOT break through by 12.5+ points). Critical for similarity retrieval to match cross-level patterns like "PM_HIGH tested 3x while OR_LOW defended 5x."
...
**Source of Truth**: 183-dim vectors stored in `gold/episodes/es_level_episodes/vectors/date=YYYY-MM-DD/episodes.npy`
...
**Within-Partition Similarity**: Episode vectors are level-agnostic. The 183-dim vector captures market structure dynamics relative to "the level being tested" without encoding which specific level it is (that's the partition key).

**Cross-Level Context**: Section A (indices 23-58) captures what's happening at ALL 6 levels simultaneously - touch counts, defended counts, and temporal recency from both directions. This enables queries like "find setups where PM_HIGH was tested 3x from below while OR_LOW was defended 5x from above."
