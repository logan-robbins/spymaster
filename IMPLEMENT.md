...
1) Define dataset entries and Avro contracts for `book_snapshot_1s`, `wall_surface_1s`, `vacuum_surface_1s`, `radar_vacuum_1s`, `physics_bands_1s`, `gex_surface_1s`, plus a **normalization calibration artifact** (`gold.hud.physics_norm_calibration`). Status: **IN PROGRESS** (book_snapshot_1s, wall_surface_1s, gex_surface_1s, vacuum_surface_1s, physics_bands_1s, physics_norm_calibration, instrument_definitions done; radar_vacuum_1s done).
2) Add silver stages in `future_mbo` to build book snapshot + wall surface from a single book reconstruction pass, and to derive `vacuum_surface_1s`, `radar_vacuum_1s`, `physics_bands_1s`. Status: **IN PROGRESS** (snapshot + wall + vacuum_surface_1s + physics_bands_1s + radar_vacuum_1s implemented).
...
1) Add bronze cache datasets and checkpoint writes for 0DTE option MBO + statistics. Status: **DONE**.
...
- \(lo_k = Q_{05}(x_k)\), \(hi_k = Q_{95}(x_k)\) computed over **08:30–09:30 (UTC-5)** across the **most recent 1 session**.
...
- `q05` (double)
- `q95` (double)
- `lookback_sessions` (int; always 1)
- `session_window` (string; always “08:30-09:30_ETC_GMT+5”)
